...
for i = 1:length(datasets)
    ...
    for j = 1:length(colors)
        ...
        println("budget = $b    lp = $lpval    bp1 apx = $bp1_ratio    bp1 mistakes = $bp1_mistakes    bp1 sat% = $bp1_sat")
    end
end
...
for i = 1:length(datasets)
    ...
    for j = 1:length(colors)
        ...
        bicrit_ratio = round(mat["bicrit_ratio"], digits=2)
        bicrit_sat = round(mat["bicrit_satisfaction"], digits=2)
        bicrit_max_colors = mat["bicrit_max_colors"]
        bicrit_budget_ratio = round(mat["bicrit_budget_ratio"], digits=2)
        println("budget = $b    lp = $lpval    bicrit apx = $bicrit_ratio    bicrit mistakes = $bicrit_mistakes    bicrit sat% = $bicrit_sat    max colors: $bicrit_max_colors    budget apx: $bicrit_budget_ratio" )
    end
end
