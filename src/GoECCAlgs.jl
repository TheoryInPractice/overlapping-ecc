...
"""
GoECCLP
Solves the Globally Budgeted Overlapping Edge-Colored Clustering LP relaxation

Input:
    EdgeList = the edge list of a LO-ECC instance
    EdgeColors = the edge colors
    n = number of vertices in the instance
    b = gloval budget for overlapping color assignments
...
Output:
    ...
    Z = The extra assignment variables
    runtime = Solver runtime
"""
function GoECCLP(EdgeList::Vector{Vector{Int64}}, EdgeColors::Array{Int64, 1}, n::Int64, b::Int64, optimalflag::Bool=false,outputflag::Int64=0)
    ...
    for i = 1:n
        @constraint(m, sum(x[i,j] for j = 1:k) >= k - z[i] - 1)
    end
...
"""
GoECCRound
Returns a rounded clustering for the GoECC objective

Inputs:
    EdgeList = the edge list of a LO-ECC instance
    EdgeColors = the edge colors
    X = the distance labels from solving the LP relaxation
    Z = the extra assignment counts from solving the LP relaxation
    LPval = the objective score from solving the LP relaxation
    b = the global budget for overlapping cluster assignments
...
function GoECCRound(EdgeList::Union{Array{Int64,2}, Vector{Vector{Int64}}}, EdgeColors::Array{Int64, 1}, X::Array{Float64,2}, Z::Array{Float64, 1}, LPval::Float64, b::Int64)
    ...
    for i = 1:n
        ...
        if length(colors) == 0 # every node gets one free assignment
    ...
    RoundBudgetScore = sum(length(c[i]) for i = 1:n) - n
